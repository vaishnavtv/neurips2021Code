## Solve the FPKE for the Van der Pol - Rayleight oscillator using baseline PINNs (large training set)
...
dx = [0.05f0; 0.05f0; 0.001f0]; # discretization size used for training
tEnd = 1.0f0; 
...
useGPU = true;
cd(@__DIR__);
saveFile = "dataTS_grid/ll_ts_vdpr_exp$(expNum).jld2";
runExp_fileName = "out_grid/log$(expNum).txt";
if runExp
    open(runExp_fileName, "a+") do io
        write(io, "Transient vdpr system with grid training in η. 2 HL with $(nn) neurons in the hl and $(suff) activation. $(maxOpt1Iters) iterations with LBFGS and then $(maxOpt2Iters) with LBFGS.  Q_fpke = $(Q_fpke). Using GPU. dx = $(dx). tEnd = $(tEnd). Not enforcing steady-state. Enforcing BC. No Diffusion. Continuity equation.
        α_ic = $(α_ic). No IC. 
...
# Van der Pol - Rayleigh oscillator Dynamics
