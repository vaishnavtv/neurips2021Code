...
using NeuralPDE, Flux, ModelingToolkit, GalacticOptim, Optim, Symbolics, JLD2, LinearAlgebra, Convex, MosekTools, ForwardDiff, DiffEqFlux
...
saveFileLoc = "data/dx1eM1_ot1Eval_vdp_$(suff)_$(nn)_ot$(otIters)_mnp$(maxNewPts)_otSh.jld2";
...
PDE_losses = Vector{Vector{Float32}}(undef, otIters + 1); PDE_losses[1] = PDE_losses1;
BC_losses = Vector{Vector{Float32}}(undef, otIters + 1); BC_losses[1] = BC_losses1;
...
for i in 1:otIters
    ...
    function newPtsFn(Cs, maxNewPts)
        nNN = size(Cs,2);
        eqnErrCs = [(pdeErrFn(Cs[:,i]))^2 for i in 1:nNN]; # equation error on collocation points Cs
    ...
    global nSteps = 0;
    PDE_losses[i+1] = Float32[]; BC_losses[i+1] = Float32[]; 
    cb_ = function (p,l)
        global nSteps = nSteps + 1;
        println("[$nSteps] Current loss is: $l")
