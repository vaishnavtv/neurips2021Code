...
using JLD2, PyPlot, NeuralPDE, ModelingToolkit, LinearAlgebra, Flux, ForwardDiff, Trapz, Printf
pygui(true);
...
otIter = 10; otIter += 1; # Check for 10th iteration (plot shown in paper)
...
function plotDistErr(RHOFine, pdeErrFine, figNum)
    ...
    figure(figNum,[8,4]); clf(); 
    subplot(1,2,1);
    pcolormesh(XXFine,YYFine,RHOFine,shading="auto", cmap = "inferno"); colorbar();
    title("Steady-State Solution (ρ)");
    xlabel("x1"); ylabel("x2");
    axis("auto"); tight_layout();

    subplot(1,2,2);
    pcolor(XXFine,YYFine,pdeErrFine,shading="auto", cmap = "inferno"); colorbar();
    if otIter < otIters + 1
        newPts = newPtsAll[otIter];
