...
if runExp
    open(runExp_fileName, "a+") do io
        write(io, "Running ll_quad_f16_ using Quadrature strategy on CPU. BC_losses coefficient: $(α_bc).
        $(nn) neurons in 1 hidden layer with $(activFunc). $(maxOpt1Iters) iterations with ADAM $(opt1.eta) and then $(maxOpt2Iters) iterations with BFGS. Using HCubatureJL. \nExperiment number: $(expNum).\n")
    end
end
...
quadrature_strategy = NeuralPDE.QuadratureTraining(quadrature_alg=HCubatureJL(), reltol=1, abstol=1e-4, maxiters=500, batch=0)
...
lbs, ubs = bcs_bounds;
bc_loss_functions = [
    NeuralPDE.get_loss_function(loss, lb, ub, eltypeθ, parameterless_type_θ, quadrature_strategy) for (loss, lb, ub) in zip(_bc_loss_functions, lbs, ubs)
]
