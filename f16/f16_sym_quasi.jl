...
using NeuralPDE, Flux, ModelingToolkit, GalacticOptim, Optim, DiffEqFlux, Symbolics, JLD2, ForwardDiff, Revise, QuasiMonteCarlo
using F16Model
...
expNum = 1;
saveFile = "data_sym/f16Sym_quasi_$(expNum).jld2";
useGPU = false;
runExp = true; # flag to check if running batch file
runExp_fileName = ("out_sym/quasi_log$(expNum).txt");
if runExp
    open(runExp_fileName, "a+") do io
        write(io, "F16 SS after some symbolic manipulation using quasi-sampling. 2 HL with $(nn) neurons in the hl and $(activFunc) activation. $(maxOpt1Iters) iterations with ADAM and then $(maxOpt2Iters) with LBFGS.  using GPU? $(useGPU). Q_fpke = $(Q_fpke).nPtsPerMB = $(nPtsPerMB). nMB = $(nMB). \n")
    end;
end
##
...
pde_loss_function = NeuralPDE.get_loss_function(_pde_loss_function, pde_bounds[1],  eltypeθ, parameterless_type_θ, strategy);
@show pde_loss_function(initθ);

bc_loss_functions = [
    NeuralPDE.get_loss_function(loss, bound, eltypeθ, parameterless_type_θ, strategy) for (loss, bound) in zip(_bc_loss_functions, bcs_bounds)
]
