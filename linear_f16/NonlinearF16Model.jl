...
using ForwardDiff, Ipopt, Symbolics
...
# Normalization of velocity/scaling
function normV(vvt)
    if vvt <= 0.1
        vt = 0.1
    else
...
function PrintStateAndControlBounds()
    ...
    stateUnits =
        ("ft", "rad", "rad", "rad", "ft/s", "rad", "rad", "rad/s", "rad/s", "rad/s")
...
function Dynamics!(xdot::Vector, x::Vector, u::Vector)
    ...
    nposd =
        ...
        W * (cphi * st * cpsi + sphi * spsi) # npos dot
    eposd =
        U * (ct * spsi) +
        V * (sphi * spsi * st + cphi * cpsi) +
        W * (cphi * st * spsi - sphi * cpsi) # epos dot
    ...
    delta_Cx_lef, delta_Cy_lef, delta_Cz_lef, delta_Cm_lef, delta_Cn_lef, delta_Cl_lef =
        Delta_lef(alpha, beta)
    ...
    delta_Cmq_lef,
    delta_Cnr_lef,
    delta_Cnp_lef = _Damping_lef(alpha)
    ...
    delta_Cn_a20_lef,
    delta_Cl_a20,
    delta_Cl_a20_lef = _AileronInfluence(alpha, beta)
    ...
    Cy_tot =
        Cy + delta_Cy_lef * dlef + dYdail * dail + delta_Cy_r30 * drud + dYdR * R + dYdP * P
    ...
    Cn_tot =
        Cn + delta_Cn_lef * dlef - Cy_tot * (xcgr - xcg) * (cbar / B) +
        dNdail * dail +
        delta_Cn_r30 * drud +
        dNdR * R +
        dNdP * P +
    ...
    rd =
        (
            Jx * N_tot + Jxz * L_tot + (Jx * (Jx - Jy) + Jxz * Jxz) * P * Q -
            Jxz * (Jx - Jy + Jz) * Q * R + Jx * Q * Heng
        ) / denom
...
function F16ModelDynamics_sym(x::Vector, u::Vector)::Vector
    # Define Constants for the Aircraft Model
    g = 32.17          # gravity, ft/s^2
    m = 636.94         # mass, slugs
    B = 30.0           # span, ft
    ...
    # Aerodynamic forces and moments
    Cx = Cx2(alpha, beta, el)
    Cy = Cy2(alpha, beta)
    Cz = Cz2(alpha, beta, el)
    Cm = Cm2(alpha, beta, el)
    Cn = Cn2(alpha, beta, el)
    Cl = Cl2(alpha, beta, el)
