...
f(x) = [x[2]; -x[1] + (1 - x[1]^2 - x[2]^2) * x[2] + Kc(x[1], x[2])];
...
function loss_function_(θ, p)
    return pde_loss_function(θ) +
           bc_loss_function_sum(θ) +
           rhoSS_loss_function(θ) #+
        #    uNorm_loss_function(θ)
end
...
if runExp
    jldsave(
        saveFile; optParam = res.minimizer, PDE_losses, BC_losses, rhoSS_losses, uNorm_losses);
end
