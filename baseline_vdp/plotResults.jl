...
using JLD2, PyPlot, VectorizedRoutines, NeuralPDE, ModelingToolkit, LinearAlgebra, Flux, Trapz, Printf, LaTeXStrings, ForwardDiff
pygui(true);
@variables x1,x2

# Load data 
activFunc = tanh; dx = 0.05;
...
function plotDistErr(figNum)
    ...
    subplot(1,2,1);
    # figure(figNum); clf();
    pcolor(XXFine,YYFine,RHOFine,shading="auto", cmap = "inferno"); colorbar();
    xlabel("x1"); ylabel("x2");
    axis("auto");
    title("Steady-State Solution (ρ)");

    subplot(1,2,2);
    pcolormesh(XXFine,YYFine,FFFine, cmap = "inferno", shading="auto"); colorbar();axis("auto");
