...
if runExp
    open(runExp_fileName, "a+") do io
        write(io, "Steady State vdp with Quadrature training. 2 HL with $(nn) neurons in the hl and $(suff) activation. $(maxOpt1Iters) iterations with ADAM and then $(maxOpt2Iters) with BFGS. Using HCubatureJL.
        Removed exponential terms in pde.
...
# driftTerm = (Differential(x1)(x2) + Differential(x2)((x2*(1 - (x1^2)) - x1)))
...
strategy = NeuralPDE.QuadratureTraining(quadrature_alg=HCubatureJL(), reltol=1e-6, abstol=1e-3, maxiters=1000, batch = 0)
