...
using JLD2, PyPlot, NeuralPDE, ModelingToolkit, LinearAlgebra, Flux, ForwardDiff, Trapz, Printf, LaTeXStrings
pygui(true);
...
=======
phi = NeuralPDE.get_phi(chain, parameterless_type_θ );
>>>>>>> 58bb720c3959d3f68d77ce3dcf5d32fd73808aae
...
for otIter in 0:otIters-1 
    ...
    println("The mean squared equation error after $(otIter-1) iterations is: $(mseEqErrVec[otIter])");
end
...
function plotDistErr(nEvalFine, RHOFine, pdeErrFine, figNum)
    ...
    println("The mean squared absolute error in the solution after $(otIter-1) iterations is:")
    ...
    subplot(1,3,1);
    pcolormesh(XXFine,YYFine,RHOFine,cmap = "inferno", shading="auto"); colorbar();
    xlabel("x1"); ylabel("x2");
    title("Prediction");
    axis("auto"); tight_layout();

    subplot(1,3,2);
    pcolormesh(XXFine,YYFine,RHOTrue, cmap ="inferno", shading="auto"); colorbar();
    xlabel("x1"); ylabel("x2");
    title("Exact");
    axis("auto"); tight_layout();

    subplot(1,3,3);
    pcolormesh(XXFine,YYFine,RHOErr,shading="auto", cmap = "inferno"); colorbar();
