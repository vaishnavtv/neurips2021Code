using QuasiMonteCarlo, Convex, MosekTools, NearestNeighbors, OptimalTransport, Primes, GoldenSequences
...
function HyperCubeSampler(xmin, xmax, N; algo=:sobol)
    ...
    elseif algo == :halton
        ...
        primeNums = [prime(i+1) for i in 1:d];
...
function ErrorBasedSampler(nPoints, nSamples, rbfX, α, pdeError, domain, Optimizer; sampler=:halton, otAlgo=:LP)
    xmin = domain[:,1];
    xmax = domain[:,2];
    ...
    d = size(rbfX, 1);
    β = findβ(rbfX, 2^d);
    T = [pdeError(errorC[:, j], rbfX[:, i], β[i]) for j = 1:size(errorC, 2), i = 1:size(rbfX,2)] * α;
    EqnError = T.^2;
